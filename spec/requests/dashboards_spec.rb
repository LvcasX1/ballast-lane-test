...
RSpec.describe 'Dashboards', type: :request do
  ...
  it 'librarian dashboard returns counts and overdue members' do
    ...
    get '/dashboard/librarian', headers: auth_headers_for(librarian)
  ...
  it 'member dashboard returns active and overdue lists' do
    ...
    get '/dashboard/member', headers: auth_headers_for(member)
    expect(response).to have_http_status(:ok)
    body = JSON.parse(response.body)
  ...
  it 'member dashboard forbidden for librarian' do
    get '/dashboard/member', headers: auth_headers_for(librarian)
    expect(response).to have_http_status(:forbidden)
  end

  it 'librarian dashboard forbidden for member' do
    get '/dashboard/librarian', headers: auth_headers_for(member)
    expect(response).to have_http_status(:forbidden)
  end
end
